...
7. **Timeout e silêncio zero**: toda chamada tem timeout de **10 segundos + 1 retentativa**; aos ~5 segundos sem retorno, o agente envia mensagem de espera ("Um instante, estou verificando no sistema."); estourado o timeout, a demora é tratada como **sem resposta** → posicionamento transparente + transbordo T9 para a fila da categoria. A conversa nunca fica sem resposta (detalhamento no documento 05, seção 2.2).
8. **Sem fontes externas em tempo real**: os agentes não navegam em sites, links ou documentos externos durante a conversa (nem no site ftw.com.br) — respondem apenas com a base de conhecimento oficial e as integrações deste catálogo. Conteúdo do site é enviado como link para o cliente abrir. Informação que só existe fora da base = lacuna (T6): posicionar, transbordar e incorporar o conteúdo à base na curadoria.
